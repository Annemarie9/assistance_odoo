# File: /content/odoo_doc17.0/fr/content/applications/inventory_and_mrp/inventory/product_management/configure.md
...
-   `Units of measure (UoM) <configure/uom>`{.interpreted-text
    role="doc"}: a standard quantity for specifying product amounts
    (e.g., meters, yards, kilograms). Enables automatic conversion
    between measurement systems in Odoo, such as centimeters to feet.
    -   *Ex: Purchasing fabric measured in meters but receiving it in
        yards from a vendor.*
-   `configure/package`{.interpreted-text role="doc"}: A physical
    container used to group products together, regardless of whether
    they are the same or different.
    -   *Ex: A box containing assorted items for delivery, or a storage
        box of two hundred buttons on a shelf.*
-   `configure/packaging`{.interpreted-text role="doc"}: groups the
    *same* products together to receive or sell them in specified
    quantities.
...
requirements.
  ...
  Product uniformity           Defined per product; saved as one `UoM (Unit of Measure)`{.interpreted-text role="abbr"} in the database                                                        Allows mixed products                                                                                                                                                        Same products only
  Flexible                     Converts between vendor/customer `UoMs (Units of Measure)`{.interpreted-text role="abbr"} and database `UoM (Unit of Measure)`{.interpreted-text role="abbr"}   Items can be added or removed from the container                                                                                                                             Quantities are fixed (e.g., always packs of 6, 12 or 24)
  ...
  Inventory tracking           Tracks product quantities within the warehouse in the specific `UoM (Unit of Measure)`{.interpreted-text role="abbr"} defined in the product form               Tracks package location and contents within the warehouse                                                                                                                    Tracks grouped quantities but not individual items\' locations
  Smooth barcode operations    Not available                                                                                                                                                   Requires scanning both the package and individual items for reception. (even if there are 30 items in a package). Can enable the `Move Entire Packages                       Scanning a packaging barcode automatically records all included units. (e.g. 1 pack = 12 units)
                                                                                                                                                                                               <inventory/product_management/move-entire-pack>`{.interpreted-text role="ref"} feature to update the package\'s contained items\' locations, when moving the package         
  ...
  Reusability                  Not applicable                                                                                                                                                  Can be disposable or reusable, configured via the `Package Use                                                                                                               Disposable only
                                                                                                                                                                                               <inventory/warehouses_storage/cluster-pack>`{.interpreted-text role="ref"} field                                                                                             
  Container weight             Not applicable                                                                                                                                                  Weight of the container itself is included in the *Shipping Weight* field of a package (`Inventory app --> Products --> Packages`{.interpreted-text role="menuselection"})   Weight of the container is defined in the *Package Type* settings
  Lot/serial number tracking   Requires manual adjustments to track `UoMs (Units of Measure)`{.interpreted-text role="abbr"} via lots (See `use case                                           Applies only to contained products                                                                                                                                           Applies to both contained products and the container
                               <inventory/product_management/lots-uom>`{.interpreted-text role="ref"} for details)                                                                                                                                                                                                                                                          
...
-   **Warehouse tracking limitations**: Odoo tracks only the total
    quantity, not the number of packagings. For instance, if a pallet
    with 12 and 24 quantities is received, Odoo records 36 quantities,
    not the pallet details.
-   **Packaging barcodes are type-specific, not unique**: Barcodes
    represent packaging types (e.g., \"pallet of 96 soap bars\") but do
    not uniquely identify individual pallets, such as Pallet \#1 or
    Pallet \#2.
...
<inventory/warehouses_storage/enable-package>`{.interpreted-text
role="ref"}, scanning a package barcode displays its contents in the
**Barcode** app.
...
### Track different units of measure in storage {#inventory/product_management/lots-uom}

A fruit juice distributor tracks multiple
`UoMs (Units of Measure)`{.interpreted-text role="abbr"} for their
operations:
...
`UoM (Unit of Measure)`{.interpreted-text role="abbr"} per product in
