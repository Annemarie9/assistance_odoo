# File: /content/odoo_doc17.0/fr/content/applications/finance/accounting/vendor_bills/deferred_expenses.md
...
role="menuselection"}. The following options are available:
...
:   By default, Odoo
    `automatically generates <vendor_bills/deferred/generate_on_validation>`{.interpreted-text
    role="ref"} the deferral entries when you post a vendor bill.
    However, you can also choose to
    `generate them manually <vendor_bills/deferred/generate_manually>`{.interpreted-text
    role="ref"} by selecting the `Manually & Grouped`{.interpreted-text
    role="guilabel"} option instead.

Amount Computation

:   Suppose a bill of \$1200 must be deferred over 12 months.

    -   The `Months`{.interpreted-text role="guilabel"} option accounts
        for \$100 each month prorated to the number of days in that
        month (e.g., \$50 for the first month if the
        `Start Date`{.interpreted-text role="guilabel"} is set to the
        15th of the month).
    -   The `Full Months`{.interpreted-text role="guilabel"} option
        considers each month started to be full (e.g., \$100 for the
        first month even if the `Start Date`{.interpreted-text
        role="guilabel"} is set to the 15th of the month); this means
        that with the `Full Months`{.interpreted-text role="guilabel"}
        option, a full \$100 is recognized in the first partial month,
        eliminating the need for a 13th month to recognize any remainder
        as would be the case when using the `Months`{.interpreted-text
        role="guilabel"} option.
    -   The `Days`{.interpreted-text role="guilabel"} option accounts
        for different amounts depending on the number of days in each
        month (e.g., \~\$102 for January and \~\$92 for February).

Generate deferral entries on validation {#vendor_bills/deferred/generate_on_validation}
---------------------------------------
...
Make sure the `Start Date`{.interpreted-text role="guilabel"} and
`End Date`{.interpreted-text role="guilabel"} fields are visible in the
`Invoice Lines`{.interpreted-text role="guilabel"} tab. In most cases,
the `Start Date`{.interpreted-text role="guilabel"} should be in the
same month as the `Bill Date`{.interpreted-text role="guilabel"}.
...
If the `Generate Entries`{.interpreted-text role="guilabel"} field is
set to `On invoice/bill validation`{.interpreted-text role="guilabel"},
Odoo automatically generates the deferral entries when the bill is
validated. Click on the `Deferred Entries`{.interpreted-text
role="guilabel"} smart button to see them.
...
`Accounting --> Reporting --> Deferred Expense`{.interpreted-text
role="menuselection"}.

To view the journal items of each account, click on the account name and
then `Journal
Items`{.interpreted-text role="guilabel"}.
...
Generate grouped deferral entries manually {#vendor_bills/deferred/generate_manually}
------------------------------------------
...
do so, set the `Generate Entries`{.interpreted-text role="guilabel"}
field in the **Settings** to `Manually & Grouped`{.interpreted-text
role="guilabel"}. Odoo then aggregates the deferred amounts in a single
entry.

At the end of each month, go to the Deferred Expenses report and click
the `Generate Entries`{.interpreted-text role="guilabel"} button. This
...
-   One dated at the end of the month which aggregates, for each
    ...
    the end of that period, a part of the deferred expense is
    recognized.
-   The reversal of this created entry, dated on the following day
    (i.e., the first day of the next month) to cancel the previous
    entry.
...
:   At the end of January, after clicking the
    `Generate Entries`{.interpreted-text role="guilabel"} button, there
    are the following entries:

    -   Entry 1 dated on the 31st January:
        -   Line 1: Expense account -1200 -600 = **-1800** (cancelling
            the total of both bills)
        -   Line 2: Expense account 100 + 50 = **150** (recognizing 1/12
            of bill A and bill B)
        -   Line 3: Deferred account 1800 - 150 = **1650** (amount that
            has yet to be deferred later on)
...
:   At the end of February, after clicking the
    ...
    -   Entry 1 dated on the 28th February:
        -   Line 1: Expense account -1200 -600 = **-1800** (cancelling
            the total of both bills)
        -   Line 2: Expense account 200 + 100 = **300** (recognizing
            2/12 of bill A and bill B)
        -   Line 3: Deferred account 1800 - 300 = **1500** (amount that
            has yet to be deferred later on)
...
:   At the end of November, after clicking the
    ...
    -   Entry 1 dated on the 30th November:
        -   Line 1: Expense account -1200 -600 = **-1800** (cancelling
            the total of both bills)
        -   Line 2: Expense account 1100 + 550 = **1650** (recognizing
            11/12 of bill A and bill B)
        -   Line 3: Deferred account 1800 - 1650 = **150** (amount that
            has yet to be deferred later on)
