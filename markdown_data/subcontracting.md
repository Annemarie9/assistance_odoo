# File: /content/odoo_doc17.0/fr/content/applications/inventory_and_mrp/manufacturing/subcontracting.md
...
::: {.cards}
::: {.card target="subcontracting/subcontracting_basic"}
...
::: {.card target="subcontracting/subcontracting_resupply"}
...
::: {.card target="subcontracting/subcontracting_dropship"}
...
checkbox next to the `Subcontracting`{.interpreted-text role="guilabel"}
setting, under the `Operations`{.interpreted-text role="guilabel"}
heading. Then, click `Save`{.interpreted-text role="guilabel"}.
...
-   On bills of materials (BoMs), the *BoM Type* field now includes a
    *Subcontracting* option. Enabling the *Subcontracting*
    `BoM (Bill of Materials)`{.interpreted-text role="abbr"} type
    designates the `BoM (Bill of Materials)`{.interpreted-text
    role="abbr"}\'s product as a subcontracted product, which means Odoo
    knows that it is produced by a subcontractor, and not by the company
    that owns the Odoo database.
-   Two subcontracting routes become available in the *Inventory* app,
    and can be assigned to specific products, on the *Inventory* tab of
    their product pages:
...
-   In the *basic* subcontracting workflow, the subcontractor is fully
    responsible for obtaining the components. This workflow is outlined
    in the `subcontracting/subcontracting_basic`{.interpreted-text
...
-   In the *Resupply Subcontractor on Order* workflow, the contracting
    ...
    `subcontracting/subcontracting_resupply`{.interpreted-text
    role="doc"} documentation.
-   In the *Dropship Subcontractor on Order* workflow, the contracting
    company purchases the components from a vendor, and has them
    delivered directly to the subcontractor. This workflow is outlined
    in the `subcontracting/subcontracting_dropship`{.interpreted-text
    role="doc"} documentation.
...
-   The cost of the required components, if provided by the contracting
    company; from here on referred to as [C]{.title-ref}.
-   The price paid to the subcontractor for the service of manufacturing
    ...
    [M]{.title-ref}.
-   The cost of shipping components to the subcontractor, and having
    them shipped back to the contracting company; from here on referred
    to as [S]{.title-ref}.
...
-   Any other associated costs, like import taxes, etc.; from here on
    referred to as [x]{.title-ref}.

Therefore, the total valuation of a subcontracted product
([P]{.title-ref}) can be represented by the following equation:
...
::: {.toctree}
subcontracting/subcontracting\_basic
subcontracting/basic\_subcontracting\_lead\_times
subcontracting/subcontracting\_resupply
subcontracting/resupply\_subcontracting\_lead\_times
subcontracting/subcontracting\_dropship
subcontracting/dropship\_subcontracting\_lead\_times
:::
