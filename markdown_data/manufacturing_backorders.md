# File: /content/odoo_doc17.0/fr/content/applications/inventory_and_mrp/manufacturing/workflows/manufacturing_backorders.md
...
immediately marked as `Done`{.interpreted-text role="guilabel"}. Order
*002* contains the five units that still need to be manufactured and is
marked as `In Progress`{.interpreted-text role="guilabel"}. Once the
...
role="menuselection"}. Select a manufacturing order with a quantity of
two or more or create one by clicking `Create`{.interpreted-text
role="guilabel"}.

If a new manufacturing order is created, select a product from the
`Product`{.interpreted-text role="guilabel"} drop-down menu and enter a
quantity of two or more in the `Quantity`{.interpreted-text
role="guilabel"} field, then click `Confirm`{.interpreted-text
role="guilabel"} to confirm the order.

After manufacturing the quantity that is being produced immediately,
enter that number in the `Quantity`{.interpreted-text role="guilabel"}
field at the top of the manufacturing order.

{.align-center}

Next, click `Validate`{.interpreted-text role="guilabel"}, and a
`You produced less than initial demand`{.interpreted-text
role="guilabel"} pop-up window appears, from which a backorder can be
created. Click `Create Backorder`{.interpreted-text role="guilabel"} to
...
role="menuselection"}, and then select the backorder manufacturing
order. If all of the remaining units are manufactured immediately,
simply click `Validate`{.interpreted-text role="guilabel"} to close the
order.
...
role="menuselection"}, enable the `Work Orders`{.interpreted-text
role="guilabel"} checkbox, and then click `Save`{.interpreted-text
role="guilabel"}.
:::
...
`MO (Manufacturing Order)`{.interpreted-text role="abbr"} for multiple
...
`Register Production`{.interpreted-text role="guilabel"} step is
reached, and then click on it to open the
`Register Production`{.interpreted-text role="guilabel"} pop-up window.
...
Do **not** click the `# Units`{.interpreted-text role="guilabel"} button
on the right side of the step. Doing so automatically registers the full
amount of units as having been produced.
:::

On the `Register Production`{.interpreted-text role="guilabel"} pop-up
window, enter the number of units produced in the
`Quantity`{.interpreted-text role="guilabel"} field. Make sure the
number entered is *less* than the number of units listed to the right of
the field. Then, click `Validate`{.interpreted-text role="guilabel"}.

{.align-center}

The pop-up window disappears and the `# Units`{.interpreted-text
role="guilabel"} button on the work order\'s card updates to reflect the
number of units produced, as a fraction of the number of units for which
the `MO (Manufacturing Order)`{.interpreted-text role="abbr"} was
originally created.

Next, click the `Mark as Done`{.interpreted-text role="guilabel"} button
...
`MO (Manufacturing Order)`{.interpreted-text role="abbr"}\'s reference
number with a [-002]{.title-ref} tag added to the end of it.

This new reference number represents the backorder
`MO (Manufacturing Order)`{.interpreted-text role="abbr"}. The original
`MO (Manufacturing Order)`{.interpreted-text role="abbr"}\'s reference
number now appears with a [-001]{.title-ref} tag added to the end of it
to distinguish it from the backorder
`MO (Manufacturing Order)`{.interpreted-text role="abbr"}.

If the original `MO (Manufacturing Order)`{.interpreted-text
role="abbr"} has no remaining work orders, it can be closed by selecting
the `All`{.interpreted-text role="guilabel"} filter in the top
navigation of the *Shop Floor* module, and then clicking `Close
Production`{.interpreted-text role="guilabel"} at the bottom of the
`MO (Manufacturing Order)`{.interpreted-text role="abbr"}\'s card.

If the original `MO (Manufacturing Order)`{.interpreted-text
role="abbr"} has remaining work orders that must be completed before it
...
`MO (Manufacturing Order)`{.interpreted-text role="abbr"} is ready to be
...
completed, the `MO (Manufacturing Order)`{.interpreted-text role="abbr"}
can be closed by clicking the `Close Production`{.interpreted-text
role="guilabel"} button at the bottom of the work order\'s card.
