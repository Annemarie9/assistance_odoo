# File: /content/odoo_doc17.0/fr/content/applications/finance/accounting/bank/bank_synchronization.md
...
`See list of supported institutions`{.interpreted-text role="guilabel"}.
...
-   **Yodlee**: Worldwide
-   `Salt Edge <bank_synchronization/saltedge>`{.interpreted-text
    role="doc"}: Worldwide
-   `Ponto <bank_synchronization/ponto>`{.interpreted-text role="doc"}:
    Europe
-   `Enable Banking <bank_synchronization/enablebanking>`{.interpreted-text
    role="doc"}: Scandinavian countries
...
`Accounting --> Lock Dates`{.interpreted-text role="menuselection"}, and
setting a date in the `Journal Entries Lock Date`{.interpreted-text
role="guilabel"} field.
:::
...
manually by clicking on the `Synchronize Now`{.interpreted-text
role="guilabel"} button on the dashboard.

Alternatively, activate the
`developer mode <developer-mode>`{.interpreted-text role="ref"}, go to
`Accounting --> Configuration --> Online Synchronization`{.interpreted-text
role="menuselection"}, select your institution, and then click the
`Fetch transactions`{.interpreted-text role="guilabel"} button.
...
Online Synchronization`{.interpreted-text role="menuselection"}, select
...
with the proxy using the `Fetch Account`{.interpreted-text
...
If you are unable to reconnect using the `Reconnect`{.interpreted-text
role="guilabel"} button, please contact the
 directly with your client id or the
reference of the error listed in the chatter.
:::

Migration process for users having installed Odoo before December 2020 {#MigrationOnlineSync}
----------------------------------------------------------------------
...
To do so, go to `Apps --> Update Apps List`{.interpreted-text
role="menuselection"}, remove the default filter in the search bar and
type [account\_online\_synchronization]{.title-ref}. You can then click
on `Install`{.interpreted-text role="guilabel"}. Finally, make sure all
...
-   All previous synchronizations are disconnected during the
    installation and will not work anymore. To view them, activate the
    `developer mode <developer-mode>`{.interpreted-text role="ref"} and
    go to
    `Accounting --> Configuration --> Online Synchronization`{.interpreted-text
    role="menuselection"}. It is not possible to resynchronize these
    connections; you have to make new ones.
-   Do not uninstall the [account\_online\_sync]{.title-ref} module,
    which is the previous module for online synchronization. The new one
    overrides it.
-   By default, the [account\_online\_synchronization]{.title-ref}
    module is installed automatically with Accounting.
:::
...
`Accounting Dashboard`{.interpreted-text role="guilabel"}, and click on
the `Synchronize Now`{.interpreted-text role="guilabel"} button.
Synchronize and fetch transactions by activating the
`developer mode <developer-mode>`{.interpreted-text role="ref"} and
going to
`Accounting --> Configuration --> Online Synchronization`{.interpreted-text
role="menuselection"}. Some providers only allow one refresh per day, so
it is possible that clicking on `Synchronize Now`{.interpreted-text
role="guilabel"} does not get your latest transactions if you already
performed such action earlier in the day.

A transaction can be visible on your bank account but not be fetched if
it has the status `Pending`{.interpreted-text role="guilabel"}. Only
transactions with the `Posted`{.interpreted-text role="guilabel"} status
...
`Accounting --> Configuration --> Online Synchronization`{.interpreted-text
role="menuselection"}, and click the `Fetch Account`{.interpreted-text
role="guilabel"} button on the connection.
...
to
`Accounting --> Configuration --> Online Synchronization`{.interpreted-text
role="menuselection"}. Open the connection you want to update your
credentials and click the `Update Credentials`{.interpreted-text
role="guilabel"} button.
