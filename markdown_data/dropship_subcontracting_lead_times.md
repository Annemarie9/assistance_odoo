# File: /content/odoo_doc17.0/fr/content/applications/inventory_and_mrp/manufacturing/subcontracting/dropship_subcontracting_lead_times.md
...
When using the
`Dropship Subcontractor on Order <subcontracting_dropship>`{.interpreted-text
role="doc"} route, a company is responsible for purchasing the necessary
...
component in the `Components`{.interpreted-text role="guilabel"} tab,
and then clicking the `oi-arrow-right`{.interpreted-text role="icon"}
`(right arrow)`{.interpreted-text role="guilabel"} button to the right
of the component.

On the product page for each component, select the
`Purchase`{.interpreted-text role="guilabel"} tab. If the vendor has not
already been added, do so now by clicking `Add a line`{.interpreted-text
role="guilabel"}, and selecting the subcontractor in the
`Vendor`{.interpreted-text role="guilabel"} column.
...
`PO (Purchase Order)`{.interpreted-text role="abbr"}. Doing so creates a
second `RfQ (Request for Quotation)`{.interpreted-text role="abbr"} to
...
the dropshipper in the `Vendor`{.interpreted-text role="guilabel"}
column.

On the `RfQ (Request for Quotation)`{.interpreted-text role="abbr"}, the
`Expected Arrival`{.interpreted-text role="guilabel"} field lists the
...
the subcontractor `PO (Purchase Order)`{.interpreted-text role="abbr"}.

The `Order Deadline`{.interpreted-text role="guilabel"} field lists the
latest date on which the `RfQ (Request for Quotation)`{.interpreted-text
role="abbr"} can be confirmed, in order for the dropshipper to deliver
the components to the subcontractor by the `Expected
Arrival`{.interpreted-text role="guilabel"} date.
...
`PO (Purchase Order)`{.interpreted-text role="abbr"}, and confirm the
purchase of the components from the dropshipper. Doing so causes a
`Dropship`{.interpreted-text role="guilabel"} smart button to appear at
the top of the page.

Click the `Dropship`{.interpreted-text role="guilabel"} smart button to
open the dropship order. This order can also be accessed from the
`Resupply`{.interpreted-text role="guilabel"} smart button that now
appears on the subcontractor `PO (Purchase Order)`{.interpreted-text
role="abbr"}.

After the dropshipper has delivered the components to the subcontractor,
click the `Validate`{.interpreted-text role="guilabel"} button at the
...
`PO (Purchase Order)`{.interpreted-text role="abbr"} for one bicycle,
with an expected delivery date of May 17th.

The `RfQ (Request for Quotation)`{.interpreted-text role="abbr"} for
...
May 10th. The `RfQ (Request for Quotation)`{.interpreted-text
role="abbr"} must be confirmed by the deadline in order for Bike Friends
...
`RfQ (Request for Quotation)`{.interpreted-text role="abbr"} on May
