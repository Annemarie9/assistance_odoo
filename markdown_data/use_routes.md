# File: /content/odoo_doc17.0/fr/content/applications/inventory_and_mrp/inventory/shipping_receiving/daily_operations/use_routes.md
...
::: {.seealso}
\- 
- `Standard routes in Odoo <../daily_operations>`{.interpreted-text
role="doc"}
:::
...
`packagings
<inventory/product_management/route-on-packaging>`{.interpreted-text
role="ref"}, and on the sales order line.
:::
...
In a
`one-step receipt route <receipts_delivery_one_step>`{.interpreted-text
role="doc"}, which uses one push rule, when a product arrives in the
...
![Push rule for the \'Receive in one step\'
route.](use_routes/push-rule.png){.align-center}

For more information about configuring rules, skip to the
`Configure rules section
<inventory/shipping_receiving/configure-rules>`{.interpreted-text
role="ref"}.
...
`need to restock
<../../warehouses_storage/replenishment/reordering_rules>`{.interpreted-text
role="doc"}.

Pull rules work backward from the demand location. For example, in a
`two-step delivery
<inventory/shipping_receiving/two-step-delivery>`{.interpreted-text
role="ref"} route, where items move from *Stock* to *Output* before
...
route.](use_routes/pull-rule.png){.align-center}
...
Configuration {#use-routes/routes-rules}
-------------
...
-   Define routes within the stock warehouse according to business
    needs, such as quality control, after-sales services, or supplier
    returns.
-   Help rental management by generating automated return moves for
    rented products.

To configure a route for a product, first, open the
`Inventory`{.interpreted-text role="guilabel"} application and go to
`Configuration --> Settings`{.interpreted-text role="menuselection"}.
Then, in the `Warehouse`{.interpreted-text role="guilabel"} section,
enable the `Multi-Step Routes`{.interpreted-text role="guilabel"}
...
The `Storage Locations`{.interpreted-text role="guilabel"} feature is
automatically activated with the `Multi-Step Routes`{.interpreted-text
...
Warehouses`{.interpreted-text role="menuselection"}. Then, open a
warehouse form. In the `Warehouse Configuration`{.interpreted-text
role="guilabel"} tab, the user can view the warehouse\'s pre-configured
routes for `Incoming Shipments`{.interpreted-text role="guilabel"} and
...
the `Incoming Shipments`{.interpreted-text role="guilabel"} and
`Outgoing Shipments`{.interpreted-text role="guilabel"} routes are set,
head to `Inventory --> Configuration
--> Routes`{.interpreted-text role="menuselection"} to see the specific
routes that Odoo generated.

{.align-center}

On the `Routes`{.interpreted-text role="guilabel"} page, click on a
route to open the route form. In the route form, the user can view which
places the route is `Applicable On`{.interpreted-text role="guilabel"}.
The user can also set the route to only apply on a specific
`Company`{.interpreted-text role="guilabel"}. This is useful for
...
::: {.seealso}
`Applicable on packagings <inventory/product_management/packaging-route>`{.interpreted-text
role="ref"}
:::
...
`Rules`{.interpreted-text role="guilabel"} for the route. Each
`Rule`{.interpreted-text role="guilabel"} has an
`Action`{.interpreted-text role="guilabel"}, a
`Source Location`{.interpreted-text role="guilabel"}, and a
`Destination Location`{.interpreted-text role="guilabel"}.
...
role="guilabel"}. Next, choose the places where this route can be
...
`Rules`{.interpreted-text role="guilabel"} of the route.
...
Categories`{.interpreted-text role="menuselection"}. Then, select the
product category and open the form. Next, click `Edit`{.interpreted-text
role="guilabel"} and under the `Logistics`{.interpreted-text
role="guilabel"} section, set the `Routes`{.interpreted-text
role="guilabel"}.
...
`Warehouses`{.interpreted-text role="guilabel"}, all the transfers
occurring inside the chosen warehouse that meet the conditions of the
route\'s rules will then follow that route.

{.align-center}

If the route is applicable on `Sales Order Lines`{.interpreted-text
role="guilabel"}, it is more or less the opposite. The route must be
manually chosen when creating a quotation. This is useful if some
products go through different routes.

Remember to toggle the visibility of the `Route`{.interpreted-text
role="guilabel"} column on the quotation/sales order. Then, the route
...
role="menuselection"} and select the desired product. Then, go to the
`Inventory`{.interpreted-text role="guilabel"} tab and under the
`Operations`{.interpreted-text role="guilabel"} section, select the
`Routes`{.interpreted-text role="guilabel"}.
...
#### Rules {#inventory/shipping_receiving/configure-rules}

The rules are defined on the route form. First, go to
`Inventory --> Configuration
--> Routes`{.interpreted-text role="menuselection"} and open the desired
route form. Next, click `Edit`{.interpreted-text role="guilabel"} and in
the `Rules`{.interpreted-text role="guilabel"} section, click on
...
`Action`{.interpreted-text role="guilabel"}:

-   `Pull From`{.interpreted-text role="guilabel"}: this rule is
    triggered by a need for the product in a specific location. The need
    can come from a sales order being validated or from a manufacturing
    order requiring a specific component. When the need appears in the
    destination location, Odoo generates a picking to fulfill this need.
-   `Push To`{.interpreted-text role="guilabel"}: this rule is triggered
    by the arrival of some products in the defined source location. In
    the case of the user moving products to the source location, Odoo
    generates a picking to move those products to the destination
    location.
-   `Pull & Push`{.interpreted-text role="guilabel"}: this rule allows
    the generation of pickings in the two situations explained above.
    This means that when products are required at a specific location, a
...
-   `Buy`{.interpreted-text role="guilabel"}: when products are needed
    at the destination location, a request for quotation is created to
    fulfill the need.
-   `Manufacture`{.interpreted-text role="guilabel"}: when products are
...
zone.](use_routes/pull-from-rule-stock-to-packing.png){.align-center}

The `Operation Type`{.interpreted-text role="guilabel"} must also be
defined on the rule. This defines which kind of picking is created from
the rule.

If the rule\'s `Action`{.interpreted-text role="guilabel"} is set to
`Pull From`{.interpreted-text role="guilabel"} or
`Pull & Push`{.interpreted-text role="guilabel"}, a
`Supply Method`{.interpreted-text role="guilabel"} must be set. The
`Supply Method`{.interpreted-text role="guilabel"} defines what happens
at the source location:

-   `Take From Stock`{.interpreted-text role="guilabel"}: the products
    are taken from the available stock of the source location.
-   `Trigger Another Rule`{.interpreted-text role="guilabel"}: the
    system tries to find a stock rule to bring the products to the
    source location. The available stock is ignored.
-   `Take From Stock, if Unavailable, Trigger Another Rule`{.interpreted-text
    role="guilabel"}: the products are taken from the available stock of
    the source location. If there is no stock available, the system
    tries to find a rule to bring the products to the source location.
...
There are three rules, all `Pull From`{.interpreted-text
role="guilabel"} rules. The `Supply Methods`{.interpreted-text
role="guilabel"} for each rule are the following:

-   `Take From Stock`{.interpreted-text role="guilabel"}: When products
    are needed in the `WH/Packing Zone`{.interpreted-text
    role="guilabel"}, *picks* (internal transfers from
    `WH/Stock`{.interpreted-text role="guilabel"} to
    `WH/Packing Zone`{.interpreted-text role="guilabel"}) are created
    from `WH/Stock`{.interpreted-text role="guilabel"} to fulfill the
    need.
-   `Trigger Another Rule`{.interpreted-text role="guilabel"}: When
    products are needed in `WH/Output`{.interpreted-text
    role="guilabel"}, *packs* (internal transfers from
    `WH/Packing Zone`{.interpreted-text role="guilabel"} to
    `WH/Output`{.interpreted-text role="guilabel"}) are created from
    `WH/Packing Zone`{.interpreted-text role="guilabel"} to fulfill the
    need.
-   `Trigger Another Rule`{.interpreted-text role="guilabel"}: When
    products are needed in `Partner
    Locations/Customers`{.interpreted-text role="guilabel"}, *delivery
    orders* are created from `WH/Output`{.interpreted-text
    role="guilabel"} to fulfill the need.
...
`Waiting Another Operation`{.interpreted-text role="guilabel"} if the
