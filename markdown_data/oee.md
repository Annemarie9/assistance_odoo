# File: /content/odoo_doc17.0/fr/content/applications/inventory_and_mrp/manufacturing/reporting/oee.md
...
`OEE (overall equipment effectiveness)`{.interpreted-text role="abbr"}
...
Since `OEE (overall equipment effectiveness)`{.interpreted-text
role="abbr"} tracks work center productivity, using it requires enabling
...
`Work Orders`{.interpreted-text role="guilabel"}, under the
...
For `OEE (overall equipment effectiveness)`{.interpreted-text
role="abbr"} to accurately reflect the percentage of fully productive
...
`BoM (Bill of Materials)`{.interpreted-text role="abbr"}. A value less
...
`Manufacturing app -->
Configuration --> Work Centers`{.interpreted-text role="menuselection"},
and select a work center. On the `General Information`{.interpreted-text
role="guilabel"} tab, enter a numerical value in the
`Time Efficiency`{.interpreted-text role="guilabel"} field.

::: {.example}
Manufacturing a *chair* product requires two operations: *cut* and
*assemble*. The product\'s `BoM (Bill of Materials)`{.interpreted-text
role="abbr"} lists an expected duration of 30 minutes for each
operation.
...
and select a work center. On the `General Information`{.interpreted-text
role="guilabel"} tab, enter a numerical value in the
`Capacity`{.interpreted-text role="guilabel"} field.

::: {.example}
A *drill station* work center has a capacity of one unit. An
`MO (manufacturing order)`{.interpreted-text role="abbr"} is confirmed
...
on the product\'s `BoM (Bill of Materials)`{.interpreted-text
role="abbr"}.
:::

### `OEE (overall equipment effectiveness)`{.interpreted-text role="abbr"} target

The `OEE (overall equipment effectiveness)`{.interpreted-text
role="abbr"} target is the goal for how much of a work center\'s
operating time should be fully productive time. It is displayed as a
percentage, and should only be set as high as [100%]{.title-ref}.

To set the `OEE (overall equipment effectiveness)`{.interpreted-text
role="abbr"} target for a work center, navigate to
`Manufacturing app -->
Configuration --> Settings --> Work Centers`{.interpreted-text
role="menuselection"}, and select a work center. On the `General
Information`{.interpreted-text role="guilabel"} tab, enter a numerical
value of [100.00]{.title-ref} or less in the
`OEE Target`{.interpreted-text role="guilabel"} field.

Calculating `OEE (overall equipment effectiveness)`{.interpreted-text role="abbr"}
----------------------------------------------------------------------------------
...
using a
`maintenance request <../../maintenance/maintenance_requests>`{.interpreted-text
role="doc"}.
...
`OEE (overall equipment effectiveness)`{.interpreted-text role="abbr"} reporting
--------------------------------------------------------------------------------

To view `OEE (overall equipment effectiveness)`{.interpreted-text
role="abbr"} reporting metrics for every work center, navigate to
`Manufacturing app
--> Reporting --> Overall Equipment Effectiveness`{.interpreted-text
role="menuselection"}. This page shows the metrics for each work center
with `OEE (overall equipment effectiveness)`{.interpreted-text
role="abbr"} data.
...
role="menuselection"}, and select a work center. At the top of the work
center\'s form, click the `fa-pie-chart`{.interpreted-text role="icon"}
`OEE`{.interpreted-text role="guilabel"} smart button.
...
either page, click the `fa-bar-chart`{.interpreted-text role="icon"}
...
`(pie chart)`{.interpreted-text role="guilabel"} button above the
displayed chart.
...
`(pivot view)`{.interpreted-text role="guilabel"} or
`oi-view-list`{.interpreted-text role="icon"} `(list
view)`{.interpreted-text role="guilabel"} buttons at the top-right
corner of the page.
